...
#### ensure that the structure of the variables is correct ####
CO_poly_all <- CO_poly_all %>% 
  dplyr::select(species, quad, year_t, area_t, survives_tplus1, nearEdge_t, area_tplus1, neighbors_5_s, neighbors_10_s, neighbors_15_s, neighbors_20_s, size_t_log, RTD_s, RDMC_s, SLA_s, SPEI_s, LDMC_s, TLP_s, SRL_s, RDiam_s, SPEI_uniform, precip_s,  size_tplus1_log, Ann.Avg.T) %>% 
  mutate(species = as.factor(species), quad = as.factor(quad), year_t = as.factor(year_t), nearEdge_t = as.factor(nearEdge_t), ann_temp_s = scale(Ann.Avg.T))
...
=======
CO_point_all <- CO_point_all %>% dplyr::select(species, quad, year, survives, nearEdge,  neighbors_10_s, neighbors_15_s, neighbors_20_s, RTD_s, RDMC_s, SLA_s, SPEI_s, LDMC_s, TLP_s, SRL_s, RDiam_s, SPEI_uniform, precip_s, Ann.Avg.T) %>% 
  mutate(species = as.factor(species), quad = as.factor(quad), year_t = as.factor(year), survives_tplus1 = as.integer(survives), nearEdge_t = as.integer(nearEdge), ann_temp_s = scale(Ann.Avg.T)) 
>>>>>>> 30a6837... tidying figure code
...
mSurvLDMC_grams_pvals <- mixed(mSurvLDMC_grams, data = CO_poly_LDMC, method = "LRT")
mSurvSLA_grams_pvals <- mixed(mSurvSLA_grams, data = CO_poly_SLA, method = "LRT")
mSurvRDMC_grams_pvals <- mixed(mSurvRDMC_grams, data = CO_poly_RDMC, method = "LRT")
mSurvRTD_grams_pvals <- mixed(mSurvRTD_grams, data = CO_poly_RTD, method = "LRT")
...
mSurvSLA_forbs_pvals <- mixed(mSurvSLA_forbs, data = CO_point_SLA, method = "LRT")
mSurvRDMC_forbs_pvals <- mixed(mSurvRDMC_forbs, data = CO_point_RDMC, method = "LRT")
...
#### comparing spp in growth analysis to spp in survival analsysis (for grams only) ####
## plot growth for each species
ggplot(data = CO_grow_TLP) +
  geom_point(aes(x = size_t_log, y = size_tplus1_log, col = species), alpha = .5) +
  geom_smooth(mapping = aes(x = size_t_log, y = size_tplus1_log, col = species, fill = species), method = "lm") +
  geom_abline(aes(intercept = 0, slope = 1), col = "grey", lty = 2) +
  facet_wrap(~species) +
  theme_classic()

## get a table of the sample size for each species in the growth dataset
par(mfrow = c(1,2))
barplot(table(CO_grow_TLP$species),
        main = "growth dataset sample size",
        las = 2)
  
## get the sample size for the survival models
barplot(table(CO_poly_TLP$species),
        main = "survival dataset sample size",
        las = 2)

## see where the gram species fall on the trait spectrum
traits_temp <- unique(CO_poly_TLP[, c("species", "TLP_s", "LDMC_s", "RDMC_s", "RTD_s")]) 

# add growth sample size
traits_temp <- left_join(traits_temp, as.data.frame(table(CO_grow_TLP$species)), by = c("species" = "Var1")) %>% 
  rename("grow_sampSize" = "Freq")
traits_temp <- left_join(traits_temp, as.data.frame(table(CO_poly_TLP$species)), 
                         by = c("species" = "Var1")) %>% 
                           rename("surv_sampSize" = "Freq")
                           
ggplot(traits_temp) +
  geom_boxplot(aes(x = species, y = TLP_s), col = "red") + 
  geom_boxplot(aes(x = species, y = LDMC_s), col = "blue") + 
  geom_boxplot(aes(x = species, y = RDMC_s), col = "green") + 
  geom_boxplot(aes(x = species, y = RTD_s), col = "orange") + 
  theme_classic()

ggplot(data = traits_temp) +
  geom_smooth(aes(y = grow_sampSize, x = LDMC_s), col = "yellow", method = "lm") + 
  geom_smooth(aes(y = surv_sampSize, x = LDMC_s), method = "lm", col = "pink") +
  geom_point(aes(y  = surv_sampSize, x = LDMC_s, col = species), pch = 12) +
  geom_point(aes(y = grow_sampSize, x = LDMC_s, col = species)) +  
  theme_classic()
