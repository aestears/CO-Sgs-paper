...
#levels(dat_text$trait) <- c("scaled(Turgor~Loss~Point)~(MPa)", "scaled(Leaf~Dry~Matter~Content)~(g/g)", "scaled(Root~Dry~Matter~Content)~(g/g)", "scaled(Average~Root~Diameter)~(cm)", expression('scaled(Specific~Leaf~Area)~(g/c'*m^'2'*')'))
...
GrowthExtraFig <- ggplot(data = GrowthDat) +
  ...
  scale_color_manual(labels = c("dry year", "wet year"), values = c("#e78ac3", "#8da0cb")) +
  scale_fill_manual(values = c("#e78ac3", "#8da0cb"), guide = FALSE) +
  geom_text(data= dat_text, mapping = aes(x = x, y = y, label = label), size = 3, fontface = "bold") +
...
```{r mainObservationsFig, fig.width=9, fig.height=10}
mainObs <- plot_grid(forbSurvFigure, gramSurvFigure, GrowthExtraFig, ncol = 3, align = "h", axis = "tb", rel_widths = c(.9,.9,1)) %>% 
  ggdraw() + draw_grob(grid::rectGrob(gp = grid::gpar(fill = "white", col = "white")) ,hjust = 0, vjust = 0, x = .07, y = .06, width = .92, height = .0225) 
...
#add_sub(as_gtable(mainObs), label = bquote("scaled(Specific Leaf Area) (g/c"*m^2*")"),x = .5, y = 3, size = 9) %>% 
#add_sub(label = bquote("scaled(Specific Leaf Area) (g/c"*m^2*")"),x = .19, y = 4, size = 9) %>% 
#add_sub(label = bquote("scaled(Specific Leaf Area) (g/c"*m^2*")"),x = .873, y = 5, size = 9)  %>% 
#ggdraw()
```
...
```{r survEffectPlots, fig.width=8, fig.height=3}
(effectsSurv <- ggarrange(AreaEffectSurv, NeighEffectSurv, NeighEffectGrowth,
          labels = c("A", "B", "C"),
          ncol = 3, nrow = 1, align ="hv"))
```

Make a figure of model results for forb survival and gram growth and survival for other traits
```{r}
#make figure for graminoid survival
...
Combine into one figure
```{r suppObservationsFig, fig.width=9, fig.height=6}
mainObs <- plot_grid(forbSurvFigure, gramSurvFigure, GrowthExtraFig, ncol = 3, align = "h", axis = "tb", rel_widths = c(.9,.9,1)) %>% 
  ggdraw() + draw_grob(grid::rectGrob(gp = grid::gpar(fill = "white", col = "white")) ,hjust = 0, vjust = 0, x = .07, y = .529, width = .25, height = .035) +
  draw_grob(grid::rectGrob(gp = grid::gpar(fill = "white", col = "white")) ,hjust = 0, vjust = 0, x = .39, y = .529, width = .25, height = .035) +
  draw_grob(grid::rectGrob(gp = grid::gpar(fill = "white", col = "white")) ,hjust = 0, vjust = 0, x = .73, y = .529, width = .25, height = .035)

mainObs <- ggdraw(add_sub(mainObs, label = bquote("scaled(Root Dry Matter Content) (g/c"*m^3*")"),x = .5, y = 9, size = 9))
mainObs <- ggdraw(add_sub(mainObs, label = bquote("scaled(Root Dry Matter Content) (g/c"*m^3*")"),x = .19, y = 10, size = 9))
mainObs <- ggdraw(add_sub(mainObs, label = bquote("scaled(Root Dry Matter Content) (g/c"*m^3*")"),x = .873, y = 11, size = 9))
mainObs
```

make a plot of SPEI intervals
```{r uniqueSPEIfig, fig.width=6, fig.height=4}
#make a fake dataset
phenDat <- data.frame(species = "Species A", month = c(1:12), flowering = c(0,0,0,0,.2,.6,.9,.9,.7,.3,0,0), xInt = 4.5, xInt2 = 11.25, xmin = 3.5, xmax = 11.25, ymin = -.9, ymax = -.2)
phenDat <- rbind(phenDat, data.frame(species = "Species B", month = c(1:12), flowering = c(0,0,0,.3,.7,.95,.8,.4,.1,0,0,0),xInt = 3, xInt2 = 9.25, xmin = 2, xmax = 9.25, ymin = -.9, ymax = -.2))
phenDat <- rbind(phenDat, data.frame(species = "Species C", month = c(1:12), flowering = c(0,0,0,0,0,.2,.5,.75,.9,.4,0,0), xInt = 5, xInt2 = 11.25, xmin = 4, xmax = 11.25, ymin = -.9, ymax = -.2))

ggplot( data = phenDat) +
  geom_smooth(aes(y = flowering, x =  month, col = species), method = 'gam', se = FALSE) +
  geom_rect(aes(xmin = xmin,xmax = xmax, ymin = ymin, ymax = ymax, col = species, fill = species)) +
  geom_vline(aes(xintercept = xInt), lty = 2, col = "grey30") +
  geom_vline(aes(xintercept = xInt2), lty = 2, col = "grey30") + 
  geom_text(aes(label = "species-level SPEI Interval", x = (xInt+3), y = -.55), colour = 
              "white") +
  facet_grid(species~.) +
  ylim(c(-1,1)) + 
  xlim(c(1,12)) +
  xlab("Month") +
  ylab("Likelihood of Flowering") +
